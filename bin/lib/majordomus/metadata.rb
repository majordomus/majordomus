...
module Majordomus
  ...
  # reads a registered container's metadata
  def container_metadata?(cname)
    begin
      meta = YAML.load(File.open("#{majordomus_data}/apps/#{cname}.yml"))
    rescue
      meta = {}
    end
    meta
  ...
  # write a yaml file containing all metadata about a registered container
  def container_metadata!(cname, meta)
    File.open("#{majordomus_data}/apps/#{cname}.yml", "w") {|f| f.write(meta.to_yaml) }
  end
  
  def container_default_metadata
    {
      "name" => "",
      "image" => "",
      "image_id" => "",
  ...
  module_function :image_name, :image_tag, :image_metadata, :image_config, :lookup_image_id, :container_metadata?, 
    :container_metadata!, :container_default_metadata, :defined_params, :defined_ports, :defined_volumes
